        ...
        while True:
            try:
                if not tshark_packet_queue.empty():
                    ...
                    if parsed_packet.get("protocol") == "ARP":
                        status = "Malicious" if parsed_packet.get("arp_mismatch") else "OK"
                        rule = "ARP spoofing" if status == "Malicious" else None
                    else:
                        status, rule = detect(parsed_packet)
                    if verbose_mode:
                        if parsed_packet.get("protocol") == "Modbus":
                            logger.info(
                            ...
                            logger.info(
                                f"[VERBOSE] ARP: {parsed_packet['IP']['src']} is-at {parsed_packet['Ethernet']['src']}"
                            )
